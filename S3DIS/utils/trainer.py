...
class ModelTrainer:
        ...
        for epoch in range(config.max_epoch):
            ...
            if miou > 63:
                ...
                save_dict = {'epoch': self.epoch,
                             ...
                             'optimizer_state_dict': self.optimizer.state_dict(),
                             'saving_path': config.saving_path}
                checkpoint_path = join(checkpoint_directory, 'best_chkp_{:04}.tar'.format(miou))
                torch.save(save_dict, checkpoint_path)
    ...
    def object_classification_validation(self, net, val_loader, config):
        ...
        acc_file = join(config.saving_path, 'acc_val.txt')
        if exists(acc_file):
            with open(acc_file, 'a') as acc_txt:
                acc_txt.write('{}\n'.format(val_ACC))
        else:
            with open(acc_file, 'w') as acc_txt:
                acc_txt.write('{}\n'.format(val_ACC))
    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        miou_file = join(config.saving_path, 'mIoU.txt')
        if exists(miou_file):
            with open(miou_file, 'a') as miou_txt:
                miou_txt.write('{}\n'.format(mIoU))
        else:
            with open(miou_file, 'w') as miou_txt:
                miou_txt.write('{}\n'.format(mIoU))
