...
def kernel_point_optimization_debug(radius, num_points, num_kernels=1, dimension=3,
    ...
    kernel_points = np.random.rand(num_kernels * num_points - 1, dimension) * diameter0 - radius0  # 为什么-1？
    while (kernel_points.shape[0] < num_kernels * num_points):
        new_points = np.random.rand(num_kernels * num_points - 1, dimension) * diameter0 - radius0  #  [100*15-1,3],元素大小为0-1之间
        kernel_points = np.vstack((kernel_points, new_points))
        d2 = np.sum(np.power(kernel_points, 2), axis=1)  # 每一个点距离点(0,0,0)的距离平方
        kernel_points = kernel_points[d2 < 0.5 * radius0 * radius0, :]  # 留下距离小于1/2半径的点
    kernel_points = kernel_points[:num_kernels * num_points, :].reshape((num_kernels, num_points, -1))  # 100*15*3
    ...
    saved_gradient_norms = np.zeros((10000, num_kernels))  # 10000*100
    old_gradient_norms = np.zeros((num_kernels, num_points))  # 100*15
    step = -1
    while step < 10000:
        ...
        A = np.expand_dims(kernel_points, axis=2)  # A: (100, 15, 1, 3)
        B = np.expand_dims(kernel_points, axis=1)  # B: (100, 1, 15, 3)
        interd2 = np.sum(np.power(A - B, 2), axis=-1)  # 每一个核点和同一个核的15个核点相减（包括自身），得距离平方
        ...
        moving_dists = np.minimum(moving_factor * gradients_norms, clip)  # clip=0.05 * radius0   两者相比，逐元素取最小值
