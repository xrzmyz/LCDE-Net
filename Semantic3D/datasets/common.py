...
def batch_grid_subsampling(points, batches_len, features=None, labels=None,
    ...
    if random_grid_orient:
        ...
        # Choose two random angles for the first vector in polar coordinates  为极坐标中的第一个矢量选择两个随机角度
        theta = np.random.rand(B) * 2 * np.pi  # B个随机数
        phi = (np.random.rand(B) - 0.5) * np.pi

        # Create the first vector in carthesian coordinates  在carthesian坐标中创建第一个矢量
        u = np.vstack([np.cos(theta) * np.cos(phi), np.sin(theta) * np.cos(phi), np.sin(phi)])

        # Choose a random rotation angle  随机角度
        alpha = np.random.rand(B) * 2 * np.pi

        # Create the rotation matrix with this vector and angle
        R = create_3D_rotations(u.T, alpha).astype(np.float32)  # 创建B个旋转矩阵
    ...
    def segmentation_inputs(self,
        ...
        for block_i, block in enumerate(arch):
            ...
            if 'pool' in block or 'strided' in block:
                ...
                # New subsampling length
                dl = 2 * r_normal / self.config.conv_radius  # 原始下采样网格大小的2倍

                # Subsampled points
                pool_p, pool_b = batch_grid_subsampling(stacked_points, stack_lengths, sampleDl=dl)  # 把一个批次的点继续下采样

                flag1 = pool_p[0] in stacked_points
                ...
                # Upsample indices (with the radius of the next layer to keep wanted density)  上采样指数（下一层的半径以保持所需的密度）
            ...
            # Reduce size of neighbors matrices by eliminating furthest point  通过消除最远点来减小相邻矩阵的大小
            conv_i = self.big_neighborhood_filter(conv_i, len(input_points))
            # print(conv_i.shape)
            pool_i = self.big_neighborhood_filter(pool_i, len(input_points))
            if up_i.shape[0] > 0:
