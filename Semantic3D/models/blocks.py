...
class Attention(nn.Module):
    ...
    def forward(self, x, num_points):
        ...
        energy = torch.mm(q, k.transpose(0, 1))  # (B, N, N)
        # energy = energy.to(device=torch.device("cpu"))
        attention = torch.softmax(energy / (num_points ** (1 / 2)), dim=1)  # (B, N, N)
        # attention = attention.to(device=torch.device("cuda:0"))

        x = torch.mm(attention, v)  # (B, N, C)
        return x
...
class SEnet(nn.Module):
    def __init__(self, in_channels):
        ...
        self.wo = nn.Linear(in_channels, in_channels)

    def forward(self, inputs):
        # 全局平均池化[h,w,c]==>[None,c]
        x = torch.mean(inputs, 0)
        x = self.wo(x)
        x = torch.sigmoid(x)
        output = torch.multiply(inputs, x)
        return output
...
class KPConv(nn.Module):
    ...
    def __init__(self, kernel_size, p_dim, in_channels, out_channels, KP_extent, radius,
        ...
        :param p_dim: dimension of the point space.点的三维坐标
        ...
        self.SA = Attention(out_channels, out_channels)
        self.SE = SEnet(out_channels)
        self.con_mlp = nn.Linear(2 * out_channels, out_channels)
    ...
    def init_KP(self):
        """
        Initialize the kernel point positions in a sphere 初始化球体中的核点位置
        ...
        K_points_numpy = load_kernels(self.radius,
                                      ...
                                      fixed=self.fixed_kernel_points)  # 15*3

        return Parameter(torch.tensor(K_points_numpy, dtype=torch.float32),
                         requires_grad=False)

    def forward(self, q_pts, s_pts, neighb_inds, x, batch, layer):
        ...
        if self.deformable:
            ...
            self.offset_features = self.offset_conv(q_pts, s_pts, neighb_inds, x, batch, layer) + self.offset_bias
        ...
        out_put = torch.sum(kernel_outputs, dim=0)
        all_feature = torch.tensor([]).to(device=torch.device("cuda:0"))
        if layer > 0:
            for i in range(len(length[layer])):
                feature_every = out_put[0:torch.sum(batch.lengths[layer][0:i + 1]), :][-batch.lengths[layer][i]:, :]
                se_feature = self.SE(feature_every)
                # outputs = self.SA(feature_every, feature_every.shape[0])
                outputs = self.SA(se_feature, se_feature.shape[0])
                all_feature = torch.cat((all_feature, outputs), 0)

            all_result = torch.add(out_put, all_feature)
...
def block_decider(block_name,  # 'simple'
                  radius,  # 0.075
                  in_dim,  # 5
                  out_dim,  # 128
                  layer_ind,  # 0
...
class UnaryBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, use_bn, bn_momentum, no_relu=False):
        ...
        # self.SA = Attention(out_dim, out_dim)
        if not no_relu:
...
class UnaryBlock1(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, use_bn, bn_momentum, no_relu=False):
        ...
        super(UnaryBlock1, self).__init__()
        self.bn_momentum = bn_momentum
        self.use_bn = use_bn
        self.no_relu = no_relu
        self.in_dim = in_dim
...
class SimpleBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        #  'simple',5,128,0.075,0,Semantic3DConfige
        """
        ...
        current_extent = radius * config.KP_extent / config.conv_radius  # 0.036

        # Get other parameters
        self.bn_momentum = config.batch_norm_momentum  # 0.02
        self.use_bn = config.use_batch_norm  # true
        self.layer_ind = layer_ind  # 0
        self.block_name = block_name  # 'simple'
        self.in_dim = in_dim  # 5
        ...
        self.KPConv = KPConv(config.num_kernel_points,
                             ...
                             KP_influence=config.KP_influence,  # 'linear'
                             aggregation_mode=config.aggregation_mode,  # 'sum'
                             deformable='deform' in block_name,
                             modulated=config.modulated)  # flase
    ...
    def forward(self, x, batch):  # x是特征

        if 'strided' in self.block_name:
            q_pts = batch.points[self.layer_ind + 1]
            s_pts = batch.points[self.layer_ind]
            neighb_inds = batch.pools[self.layer_ind]
            layer = self.layer_ind + 1
        else:
            q_pts = batch.points[self.layer_ind]
            s_pts = batch.points[self.layer_ind]
            neighb_inds = batch.neighbors[self.layer_ind]
            layer = self.layer_ind

        x = self.KPConv(q_pts, s_pts, neighb_inds, x, batch, layer)
        # print('#######################')
        # print(x)
        # print(x.shape)
        # print('#######################')
...
# class ResnetBottleneckBlock1(nn.Module):
#
#     def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
#         """
#         Initialize a resnet bottleneck block.
...
#         :param config: parameters
#         """
#         super(ResnetBottleneckBlock1, self).__init__()
#
#         # get KP_extent from current radius
#         current_extent = radius * config.KP_extent / config.conv_radius
#
